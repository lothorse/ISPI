...
int outputValue(int sensorNum){ //function that takes in an index and returns the reading of the corresponding sensor/analogue pin.
  ...
  int knob_Normalised = map(knobValue, knobHighLow[1], knobHighLow[0], 0, 50);
  int lowerThreshold = 10+knob_Normalised;
  int higherThreshold = 127+knob_Normalised;
  int output;
  if(brightness < lowerThreshold){ //set the minimum number to be registered as significant. Here, anything below 20 will be ignored as noise.
    output = 0;
  }
